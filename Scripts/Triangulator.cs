 ...
 *  The algorithm supports simply connected convex polygons
 *  , but not nonconvex or multiply connected polygons,
 *  or multiple polygons at once.
 *  Reference <see cref="http://wiki.unity3d.com/index.php?title=Triangulator"/>
 * ---------------------------------------------------------------------- */
...
public class Triangulator
{
    ...
    private float Area()
    {
        ...
        for (int p = n - 1, q = 0; q < n; p = q++) {
            var pval = _mPoints[p];
            var qval = _mPoints[q];
            A += 0.5f*(pval.x * qval.y - qval.x * pval.y);
        }
        return A;
    }
